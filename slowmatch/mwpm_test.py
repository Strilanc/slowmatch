...
def alternating_tree_builder(region_maker: Callable[[int], GraphFillRegion]):
    def make_tree(
        ...
        if root:
            ...
            node = AltTreeNode(
                ...
                inner_outer_edge=CompressedEdge(
                    loc_from=DetectorNode(loc=source1),
                    loc_to=DetectorNode(loc=source2),
                    obs_mask=0,
                    distance=1
                )
            )
        alt_tree_edge = AltTreeEdge(
            node=node,
            edge=CompressedEdge(
                loc_from=DetectorNode(loc=parent_source),
                loc_to=DetectorNode(loc=child_source),
                obs_mask=0,
                distance=1
            )
        )
...
def region_hit_region_builder(fill: RecordingFlooder):
    ...
    def get_region_hit_region(r1: int, r2: int, s1: int, s2: int) -> RegionHitRegionEvent:
        return RegionHitRegionEvent(
            ...
            time=1,
            edge=CompressedEdge(
                loc_from=DetectorNode(loc=s1) if s1 is not None else None,
                loc_to=DetectorNode(loc=s2) if s2 is not None else None,
                obs_mask=0,
                distance=1
            )
        )
    ...
    def get_blossom_cycle_edge(br: int, s1: int, s2: int) -> RegionEdge:
        return RegionEdge(
            ...
            edge=CompressedEdge(
                loc_from=DetectorNode(s1),
                loc_to=DetectorNode(s2),
                obs_mask=0,
                distance=1
            )
        )
